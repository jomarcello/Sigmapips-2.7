        ...
        try:
            ...
            if len(parts) >= 4:
                ...
                if context and hasattr(context, 'user_data'):
                    ...
                    user_str_id = str(update.effective_user.id)
...
"""
    ...
    async def _store_original_signal_page(self, update, context=None, instrument=None, signal_id=None):
        """
        Store the original signal page information for later retrieval
        
        Args:
            update: The update object containing the message
            context: The context object to store data in
        ...
        try:
            if not context or not hasattr(context, 'user_data'):
                logger.error("Cannot store original signal page: context or user_data not available")
                return False
                
            user_id = update.effective_user.id
            user_str_id = str(user_id)
            query = update.callback_query if hasattr(update, 'callback_query') else None
            message = query.message if query else update.message if hasattr(update, 'message') else None
            
            # Create original signal data structure
            original_signal = {
                'instrument': instrument,
                'signal_id': signal_id,
            ...
            if signal_id and hasattr(self, 'user_signals') and user_str_id in self.user_signals and signal_id in self.user_signals[user_str_id]:
            ...
            # If we have Redis available, store the original signal there for persistence
            if hasattr(self, 'db') and self.db and self.db.using_redis:
                try:
                    redis_key = f"original_signal:{user_str_id}:{signal_id}"
                    self.db.redis.setex(redis_key, 24 * 60 * 60, json.dumps(original_signal))  # Store for 24 hours
                    logger.info(f"Stored original signal in Redis: {redis_key}")
                except Exception as redis_error:
                    logger.error(f"Error storing original signal in Redis: {str(redis_error)}")
    ...
    async def _get_original_signal_page(self, update, context=None, signal_id=None):
        """
        Retrieve the original signal page information
        
        Args:
            update: The update object containing the message
            context: The context object to retrieve data from
            signal_id: Optional signal ID to retrieve
            
        Returns:
            dict: Original signal page data or None if not found
        """
        try:
            user_id = update.effective_user.id
            user_str_id = str(user_id)
            
            # First check if we have the original signal stored in context
            ...
            if context and hasattr(context, 'user_data') and 'original_signal' in context.user_data:
                ...
                # If signal_id is provided and doesn't match, ignore this result
                if signal_id and original_signal.get('signal_id') != signal_id:
                    original_signal = None
            
            # If not in context, try to get from Redis if available
            if not original_signal and hasattr(self, 'db') and self.db and self.db.using_redis:
                # Use provided signal_id or try to get from context
                signal_id_to_use = signal_id
                if not signal_id_to_use and context and hasattr(context, 'user_data'):
                    signal_id_to_use = context.user_data.get('signal_id_backup') or context.user_data.get('signal_id')
                
                if signal_id_to_use:
                    try:
                        redis_key = f"original_signal:{user_str_id}:{signal_id_to_use}"
                        original_signal_json = self.db.redis.get(redis_key)
                        if original_signal_json:
                            original_signal = json.loads(original_signal_json)
                            logger.info(f"Retrieved original signal from Redis: {original_signal}")
                    except Exception as redis_error:
                        logger.error(f"Error retrieving original signal from Redis: {str(redis_error)}")
    ...
    async def _cleanup_old_signals(self, max_age_days=7):
        """
        Clean up old signal data from Redis to prevent memory issues
        
        Args:
            max_age_days: Maximum age in days for signals to keep
            
        Returns:
            int: Number of signals cleaned up
        """
        if not hasattr(self, 'db') or not self.db or not self.db.using_redis:
            logger.info("Redis not available, skipping signal cleanup")
            return 0
            
        try:
            # Get all keys matching the original signal pattern
            pattern = "original_signal:*"
            keys = self.db.redis.keys(pattern)
            
            if not keys:
                logger.info("No signal keys found for cleanup")
                return 0
                
            logger.info(f"Found {len(keys)} signal keys to check for cleanup")
            ...
            for key in keys:
                try:
                    # Get the signal data
                    signal_json = self.db.redis.get(key)
                    if not signal_json:
                        continue
                    ...
                    if 'timestamp' in signal_data:
                        ...
                        # Compare timestamps
                        if signal_time < cutoff_iso:
                            # Delete old signal
                            self.db.redis.delete(key)
                            deleted_count += 1
                            logger.debug(f"Deleted old signal key: {key}")
                except Exception as e:
                    logger.error(f"Error processing signal key {key} for cleanup: {str(e)}")
            
            if deleted_count > 0:
                logger.info(f"Cleaned up {deleted_count} old signal keys from Redis")
