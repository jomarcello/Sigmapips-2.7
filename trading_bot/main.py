...
class TelegramService:
    ...
    async def process_signal(self, signal_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not hasattr(self, 'signals_dir'):
                ...
                try:
                    ...
                    for admin_id in self.admin_users:
                        ...
                        if hasattr(self, 'db') and self.db:
                            ...
                            logger.info(f"Stored signal {signal_id} in database for admin {admin_id}")
                except Exception as e:
                    logger.error(f"Error sending test signal to admin: {str(e)}")
...
To continue using Sigmapips AI and receive trading signals, please reactivate your subscription by clicking the button below.
    ...
    async def _store_original_signal_page(self, update: Update, context=None, instrument=None, signal_id=None):
        """Store information about the original signal page to return to later"""
    ...
    async def back_to_signal_analysis_callback(self, update: Update, context=None) -> int:
        ...
        try:
            ...
            # Error recovery - try to go back to signal if possible
            try:
                if context and hasattr(context, 'user_data') and (
                    context.user_data.get('signal_id_backup') or 
                    context.user_data.get('signal_instrument_backup')
                ):
                    # We have signal data, try to go back to the signal
                    return await self.back_to_signal_callback(update, context)
                else:
                    # No signal data, go to main menu
                    await query.edit_message_text(
                        text="An error occurred. Please try again from the main menu.",
                        reply_markup=InlineKeyboardMarkup(START_KEYBOARD)
                    )
